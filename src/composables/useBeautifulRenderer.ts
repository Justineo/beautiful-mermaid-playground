import { DEFAULTS, THEMES, renderMermaidASCII, renderMermaidSVG } from 'beautiful-mermaid'
...
type RenderOptions = NonNullable<Parameters<typeof renderMermaidSVG>[1]>
...
function resolveThemeTokenValues(config: BeautifulRenderConfig): ThemeTokenValues {
  const baseTheme = THEMES[config.diagramTheme] ?? DEFAULTS
...
function renderSvgWithFallback(
  ...
  try {
    return {
      svg: renderMermaidSVG(source, renderOptions),
      usedFallback: false,
    }
  } catch (error: unknown) {
  ...
  try {
    return {
      svg: renderMermaidSVG(source, safeOptions),
      usedFallback: true,
    }
  } catch (error: unknown) {
  ...
  return {
    svg: renderMermaidSVG(originalSource, safeOptions),
    usedFallback: true,
  }
}
...
function applyCornerStyle(
  ...
  applyTopOnlyRadius('.subgraph > rect:nth-of-type(2)', set.subgraphHeader, 'subgraph-header-shape')
...
export function useBeautifulRenderer(
  ...
  function renderNow(): void {
    ...
    const renderId = renderState.value.renderId + 1
    ...
    try {
      ...
      if (config.value.outputMode === 'svg') {
        const result = renderSvgWithFallback(source, originalSource, renderOptions)
